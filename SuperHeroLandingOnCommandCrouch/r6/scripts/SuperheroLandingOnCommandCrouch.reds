// SuperheroLandingOnCommandCrouch, Cyberpunk 2077 mod that improves Superhero Landing
...
@replaceMethod(AirHoverDecisions)
  ...
  protected const func EnterCondition(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    if !this.m_hasStatusEffect && !scriptInterface.IsActionJustTapped(n"ToggleCrouch") && m_maxSuperheroFallHeight {
      return false;
    }
    else {
    if !this.m_hasStatusEffect && scriptInterface.IsActionJustTapped(n"ToggleCrouch") && !m_maxSuperheroFallHeight {
    ...
    if scriptInterface.IsActionJustTapped(n"ToggleCrouch") && !m_maxSuperheroFallHeight { 
      return true;
    };
    return false;
  };
